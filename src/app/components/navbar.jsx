    ...
    <nav className="sticky top-0 z-40 w-full bg-[#0a0a0a]/90 backdrop-blur-md border-b border-white/10">
      ...
      <div className="px-4 md:px-8 py-2.5 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-0.5">
          {NAV_LINKS.map(({ label, href }) => (
            <a
              key={href}
              href={href}
              className="text-xs sm:text-sm font-medium text-gray-400 hover:text-white hover:bg-white/10 px-2.5 sm:px-3 py-1.5 rounded-xl transition-colors duration-150 whitespace-nowrap"
            >
              {label}
            </a>
          ))}
        ...
        <button
          ...
          className="md:hidden p-2 hover:bg-white/10 rounded-xl transition-colors duration-150 shrink-0"
        >
          {searchOpen ? (
            <svg
        ...
        <div className="relative shrink-0">
          <button
            ...
            className="flex items-center gap-2 hover:bg-white/10 rounded-xl px-2 py-1.5 transition-colors duration-150"
          >
            {avatarUrl ? (
              <img
                src={avatarUrl}
      ...
      {searchOpen && (
        <div className="md:hidden px-4 pb-3 border-t border-white/10 pt-3 bg-[#0a0a0a]">
          <SearchBar onSearch={handleSearch} />
        </div>
      )}
    </nav>
